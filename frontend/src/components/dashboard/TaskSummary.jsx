...
export default function TaskSummary({ stats }) {
  ...
  return (
    <div className="bg-card rounded-2xl border border-borderSubtle p-6 shadow-card hover:shadow-premium transition-all flex flex-col justify-between h-full">
      <div>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-base font-bold text-textPrimary">Task Status Summary</h3>
            <p className="text-xs text-textSecondary mt-0.5">Distribution across sprint backlog</p>
          </div>
          <span className="text-xs font-bold px-3 py-1 rounded-full bg-blue-50 text-primary border border-blue-200 shadow-2xs">
            {completionRate}% Completed
          </span>
        </div>

        {/* Global Stacked Progress Bar */}
        <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden flex my-4 shadow-inner">
          <div
            ...
            <div key={item.label} className="space-y-1.5">
              <div className="flex items-center justify-between text-xs">
                ...
                <div className="flex items-center gap-2 font-mono">
                  <span className="font-bold text-textPrimary">{item.count}</span>
                  <span className="text-textSecondary text-[11px]">({item.pct}%)</span>
                </div>
              </div>
              <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                <div
                  className={`h-full ${item.barColor} transition-all duration-500 ease-out rounded-full`}
      ...
      <div className="mt-6 pt-4 border-t border-borderSubtle flex items-center justify-between text-xs text-textSecondary">
        <span>Active Workstream Scope</span>
        <span className="font-bold text-textPrimary text-sm">{total} Tasks Total</span>
      </div>
    </div>
  );
}
