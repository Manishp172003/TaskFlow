...
export default function Header({
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-borderSubtle bg-card px-4 sm:px-6 shadow-subtle">
      ...
      <div className="flex items-center gap-3 sm:gap-4 flex-1 max-w-lg">
        ...
        <div className="relative w-full max-w-xs sm:max-w-sm">
          ...
          <input
            ...
            className="w-full rounded-lg border border-borderSubtle bg-slate-50 py-1.5 pl-9 pr-14 text-sm text-textPrimary placeholder:text-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all"
          />
          <div className="absolute inset-y-0 right-0 flex items-center pr-2.5 pointer-events-none">
            <kbd className="hidden sm:inline-block px-1.5 py-0.5 text-[10px] font-semibold text-slate-400 bg-white border border-borderSubtle rounded shadow-2xs">
              ⌘K
            </kbd>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        <div className="relative">
          <button
            ...
            className="relative p-2 rounded-lg text-textSecondary hover:bg-slate-100 hover:text-textPrimary transition-colors focus:outline-none"
            title="Notifications"
          >
        ...
        <div className="relative">
          ...
          >
            {user?.avatar ? (
              <img
                src={user.avatar}
                alt={user.name}
                className="w-8 h-8 rounded-full object-cover border border-borderSubtle ring-1 ring-slate-200"
              />
            ) : (
          ...
          {dropdownOpen && (
            <>
              ...
              />
              <div className="absolute right-0 mt-2 w-56 rounded-xl border border-borderSubtle bg-card shadow-lg z-50 py-1.5 animate-in fade-in slide-in-from-top-2">
